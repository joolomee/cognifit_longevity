...
(function() {
  ...
  var ORB_CONFIGS = [
    ...
    { selector: '.s-validation', orbs: [
      { w: 720, h: 720, left: '78%', top: '22%', color: 'rgba(26,115,232,0.10)', cls: 'a' },
      { w: 460, h: 460, left: '6%',  top: '68%', color: 'rgba(90,40,200,0.06)',  cls: 'b' },
      { w: 320, h: 320, left: '48%', top: '82%', color: 'rgba(0,180,200,0.05)',  cls: 'c' }
    ]},
...
/* ═══════════════════════════════════════════════════════════════════════
   HOTFIX v2026.04.24-r3 — Runtime DOM repairs
   ═══════════════════════════════════════════════════════════════════════ */
(function(){
  'use strict';
  ...
  function runFixes(){
    try {
      /* ── 1. HERO H1 — force clean 3-span structure regardless of current state ── */
      var h1 = document.querySelector('.hero-h1, h1.hero-h1');
      if (h1) {
        var currentText = h1.textContent.trim();
        var hasSpans = h1.querySelectorAll('[data-i18n^="hero.h1."]').length === 3;
        if (!hasSpans || h1.textContent.indexOf('Sistema de Longevidade Cognitiva') > -1) {
          /* Rebuild H1 with 3 clean spans */
          h1.innerHTML =
            '<span data-i18n="hero.h1.line1">Live Longer.</span><br>' +
            '<span data-i18n="hero.h1.line2">Think Better.</span><br>' +
            '<span class="hero-shimmer-soft" data-i18n="hero.h1.line3">Stay Independent.</span>';
          /* Retrigger i18n if available */
      ...
      /* ── 2. HERO BADGE — ensure separated from H1, not duplicated ── */
      var badge = document.querySelector('.hero-badge');
      if (badge) {
        /* Make sure badge has only its own content */
        var badgeText = badge.querySelector('span[data-i18n="hero.badge"]');
        if (!badgeText) {
          var dot = badge.querySelector('.badge-dot');
          var spans = badge.querySelectorAll('span');
          spans.forEach(function(s){
            if (!s.hasAttribute('data-i18n') && s.textContent.trim()) {
              s.setAttribute('data-i18n', 'hero.badge');
            }
          });
        }
      }

      /* ── 3. DASHBOARD broken image — hide if no valid src or 404 ── */
      document.querySelectorAll('img[alt*="dashboard preview" i], img[alt*="dashboard" i]').forEach(function(img){
        if (!img.src || img.src === '' || img.src.indexOf('about:blank') > -1) {
      ...
      /* ── 4. PT language button — ensure visible styling ── */
      var navR = document.querySelectorAll('nav .nav-r > span, #nav .nav-r > span');
      navR.forEach(function(s){
        if (s.textContent.trim().length <= 4 && !s.classList.contains('styled-lang')) {
          s.classList.add('styled-lang');
          s.style.cssText = 'background:rgba(255,255,255,.08);color:#fff;padding:6px 12px;border-radius:8px;border:1px solid rgba(255,255,255,.18);font-size:12px;font-weight:700;letter-spacing:.04em;cursor:pointer;min-height:34px;display:inline-flex;align-items:center;';
        }
      });

      /* ── 5. Prevent H1 from having eyebrow text — remove any text node before spans ── */
      if (h1) {
        var textNodes = [];
        h1.childNodes.forEach(function(n){
          if (n.nodeType === 3 && n.textContent.trim() && n.textContent.indexOf('Sistema') === 0) {
            textNodes.push(n);
          }
        });
      ...
      console.warn('[CogniFit hotfix r3]', e);
    }
  }

  /* Run on DOM ready + after i18n apply */
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', runFixes);
  } else {
    runFixes();
  }
  /* Rerun after 300ms and 1s to catch late i18n changes */
